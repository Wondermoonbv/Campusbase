...
export function ContractFormDialog({ open, onOpenChange, contract, onSave }: ContractFormDialogProps) {
  ...
  const [form, setForm] = useState({
    school_id: "", contract_type: "partnership" as string, start_date: "", end_date: "",
    renewal_date: "", status: "in onderhandeling" as string, value: "", description: "",
    document_url: "", notes: "", linked_event_ids: [] as string[], file: null as File | null,
  });
  ...
  useEffect(() => {
    if (open) {
      if (contract) {
        setForm({
          school_id: contract.school_id, contract_type: contract.contract_type,
          start_date: contract.start_date, end_date: contract.end_date,
          renewal_date: contract.renewal_date, status: contract.status,
          value: contract.value?.toString() || "", description: contract.description || "",
          document_url: contract.document_url || "", notes: contract.notes || "",
          linked_event_ids: contract.linked_event_ids || [], file: null,
        });
      } else {
        setForm({
          school_id: "", contract_type: "partnership", start_date: "", end_date: "",
          renewal_date: "", status: "in onderhandeling", value: "", description: "",
          document_url: "", notes: "", linked_event_ids: [], file: null,
        });
      }
    }
  }, [open, contract]);
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={3} placeholder="Wat omvat dit contract? Inhoud, afspraken, deliverables..." />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Waarde (€)</Label>
              <Input type="number" value={form.value} onChange={(e) => update("value", e.target.value)} placeholder="0" />
            </div>
            <div>
            ...
            <div className="mt-1">
              {form.file ? (
                <div className="flex items-center gap-2 p-2.5 border border-border rounded-lg bg-muted/20">
                  ...
                  <span className="text-xs text-muted-foreground tabular-nums">{(form.file.size / 1024).toFixed(0)} KB</span>
                  <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setForm((p) => ({ ...p, file: null }))}>
                    <X className="h-3.5 w-3.5" />
                  </Button>
                </div>
              ) : (
                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full flex items-center justify-center gap-2 p-4 border-2 border-dashed border-border rounded-lg text-sm text-muted-foreground hover:border-primary/40 hover:text-foreground transition-colors cursor-pointer">
                  <Upload className="h-4 w-4" /> Klik om een PDF te uploaden
                </button>
              )}
              <input ref={fileInputRef} type="file" accept=".pdf" className="hidden" onChange={(e) => {
                const file = e.target.files?.[0] || null;
                if (file && file.type !== "application/pdf") { toast.error("Alleen PDF-bestanden zijn toegestaan."); return; }
                setForm((p) => ({ ...p, file }));
              }} />
            </div>
          </div>
            ...
            <div className="border border-border rounded-lg max-h-40 overflow-y-auto p-2 space-y-1.5">
              {relevantEvents.length === 0 ? (
                ...
                relevantEvents.map((event) => (
                  <label key={event.id} className="flex items-center gap-2 text-sm p-1.5 rounded hover:bg-muted/40 cursor-pointer">
                    <Checkbox checked={form.linked_event_ids.includes(event.id)} onCheckedChange={() => toggleEvent(event.id)} />
                    <span className="flex-1">{event.name}</span>
                    <span className="text-xs text-muted-foreground">{new Date(event.date).toLocaleDateString("nl-BE")}</span>
                  </label>
                ))
              )}
            </div>
