...
export function EventFormDialog({ open, onOpenChange, event, onSave }: EventFormDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const isDraftStatus = form.status === "option" || form.status === "declined";

    if (isDraftStatus && !derivedOrganisatorId) {
      toast.error("Selecteer een organisatie.");
      return;
    }

    if (!isDraftStatus && !cpValid) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FormSection title="Basis" collapsible={false}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div><Label>Type *</Label><Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="jobbeurs">Jobbeurs</SelectItem><SelectItem value="workshop">Workshop</SelectItem><SelectItem value="event">Event</SelectItem></SelectContent></Select></div>
              <div><Label>Status *</Label><Select value={form.status} onValueChange={(v) => setForm({ ...form, status: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="option">Option</SelectItem><SelectItem value="declined">Declined</SelectItem><SelectItem value="gepland">Gepland</SelectItem><SelectItem value="bevestigd">Bevestigd</SelectItem><SelectItem value="afgelopen">Afgelopen</SelectItem><SelectItem value="geannuleerd">Geannuleerd</SelectItem></SelectContent></Select></div>
            </div>
          ...
          <Button type="submit" className="w-full" disabled={(form.status !== "option" && form.status !== "declined" && !cpValid && cpEntries.length > 0) || !timesValid}>
