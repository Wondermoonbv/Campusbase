...
export function ProgramFormDialog({ open, onOpenChange, program, schoolId, onSave }: ProgramFormDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      if (program) { setForm({ name: program.name, organisatie_id: program.organisatie_id, faculty: program.faculty, study_level: program.study_level, field_of_study: program.field_of_study, student_count: program.student_count?.toString() ?? "" }); }
      else { setForm({ name: "", organisatie_id: schoolId ?? "", faculty: "", study_level: "bachelor", field_of_study: "", student_count: "" }); }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const saved: Program = { ...(program?.id ? { id: program.id } : {}), name: sanitized.name, organisatie_id: sanitized.organisatie_id, faculty: sanitized.faculty, study_level: sanitized.study_level as Program["study_level"], field_of_study: sanitized.field_of_study, student_count: sanitized.student_count ? Number(sanitized.student_count) : null } as Program;
    onSave?.(saved);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Annuleren</Button><Button type="submit" disabled={!form.name || !form.organisatie_id}>{isEdit ? "Opslaan" : "Toevoegen"}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
