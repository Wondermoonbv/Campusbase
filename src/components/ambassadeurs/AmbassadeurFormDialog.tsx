...
export function AmbassadeurFormDialog({ open, onOpenChange, ambassadeur, onSave }: Props) {
  ...
  useEffect(() => {
    if (open) {
      setForm(ambassadeur ? {
        full_name: ambassadeur.full_name,
        email: ambassadeur.email,
        phone: ambassadeur.phone ?? "",
        department: ambassadeur.department ?? "",
        notes: ambassadeur.notes ?? "",
        is_active: ambassadeur.is_active ?? true,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ ...(ambassadeur ? { id: ambassadeur.id } : {}), ...form });
  };
