...
export default function ContactenPage() {
  const { contacten, upsertContact, deleteContact } = useContacten();
  ...
  const schoolMap = Object.fromEntries(scholen.map((s) => [s.id, s]));
    ...
    <div className="page-container animate-fade-in-up">
      ...
      <div className="md:hidden space-y-3">
        {filtered.length === 0 ? (
          ...
          return (
            <div key={c.id} className="surface-card p-4 space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="font-medium text-sm">{c.name}</span>
                {canEdit && (
                  <div className="flex gap-0.5">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditContact(c); setDialogOpen(true); }}><Edit className="h-3.5 w-3.5" /></Button>
