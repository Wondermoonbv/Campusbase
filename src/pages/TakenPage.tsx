...
export default function TakenPage() {
  const { taken, upsertTask, deleteTask: deleteTaskMutation } = useTaken();
  ...
  const filterTasks = (status: TaskStatus | "active") => {
    return taken.filter((t) => {
      if (status === "active") { if (t.status === "afgerond") return false; } else if (t.status !== status) return false;
          ...
          <TableBody>{sorted.length === 0 ? <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Geen taken gevonden.</TableCell></TableRow> : sorted.map((task) => {
            ...
            return (
              <TableRow key={task.id} className={done ? "opacity-60" : ""}>
                ...
                <TableCell><div className="flex gap-1"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onEdit(task)}><Pencil className="h-3.5 w-3.5 text-muted-foreground" /></Button><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onDelete(task)}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button></div></TableCell>
              </TableRow>
            );
          })}</TableBody></Table>
      </div>
