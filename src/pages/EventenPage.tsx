      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <div className="flex border border-border rounded overflow-hidden">
            <button onClick={() => setView("list")} className={`px-3 py-2 sm:py-1.5 text-sm ${view === "list" ? "bg-primary text-primary-foreground" : "bg-card text-foreground hover:bg-muted"}`}>
              <List className="h-4 w-4" />
            </button>
            <button onClick={() => setView("calendar")} className={`px-3 py-2 sm:py-1.5 text-sm ${view === "calendar" ? "bg-primary text-primary-foreground" : "bg-card text-foreground hover:bg-muted"}`}>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
          ...
          <div className="grid grid-cols-2 sm:flex gap-2 sm:gap-3">
            <Select value={filterType} onValueChange={setFilterType}>
              <SelectTrigger className="w-full sm:w-[180px] h-10 sm:h-9"><SelectValue placeholder="Type" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger className="w-full sm:w-[150px] h-10 sm:h-9"><SelectValue placeholder="Status" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={filterFieldOfStudy} onValueChange={setFilterFieldOfStudy}>
              <SelectTrigger className="w-full sm:w-[180px] h-10 sm:h-9 col-span-2 sm:col-span-1"><SelectValue placeholder="Studiedomein" /></SelectTrigger>
              <SelectContent>
      ...
      {view === "list" ? (
        <>
          ...
          <div className="block md:hidden space-y-2">
            {sorted.length === 0 ? (
              <div className="surface-card p-6 text-center text-sm text-muted-foreground">Geen evenementen gevonden.</div>
            ) : (
              sorted.map((ev) => (
                <div
                  key={ev.id}
                  ...
                  <div key={ev.id} className="surface-card p-3 sm:p-4 flex items-center justify-between cursor-pointer hover:bg-muted/30 active:scale-[0.99] transition-[transform,colors]" onClick={() => navigate(`/evenementen/${ev.id}`)}>
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="text-center min-w-[40px] sm:min-w-[48px]">
                        <div className="text-xl sm:text-2xl font-semibold tabular-nums">{new Date(ev.date).getDate()}</div>
